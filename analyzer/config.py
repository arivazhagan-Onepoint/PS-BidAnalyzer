...
# --- NoBid knowledge maintenance (analyzer.maintain_nobids) -----------------
# A separate, scheduled maintenance flow (NOT the per-tender analysis) keeps the
# NoBid knowledge current in two steps:
#   Step 1  extract: sync rows whose Bid Qualification exactly matches this value
#           from the main tab into the NOBIDS_SHEET_NAME tab (deduped).
#   Step 2  distil : consolidate those human reasons into general NoBid decision
#           heuristics written to NOBID_PATTERNS_FILE, which the analysis prompt
#           then injects as decision precedent.
# Only human-set NoBid(Human) rows feed the distillation — never NoBid(AI) — to
# avoid a self-reinforcing feedback loop of the analyzer learning from itself.
COPY_TO_NOBIDS_STATUS = "NoBid(Human)"

# Distilled NoBid heuristics artifact (generated by Step 2, read by the analyzer,
# cached like ONEPOINT_CONTEXT_FILE). Absent/empty => analysis injects nothing.
...
# than reusing ANALYZER_MAX_TOKENS (700). Reuses ANALYZER_MODEL.
NOBID_DISTILL_MAX_TOKENS      = 3000
NOBID_DISTILL_TEMPERATURE     = 0.3
NOBID_DISTILL_THINKING_BUDGET = 0
...
# If fewer than NOBID_MIN_EXAMPLES distinct genuine reasons remain, Step 2 skips
# regeneration and keeps the existing NOBID_PATTERNS_FILE untouched.
NOBID_JUNK_MARKERS = ("test",)
NOBID_MIN_EXAMPLES = 5


def genuine_nobid_reasons(reasons) -> list:
    """Distinct, non-junk NoBid reasons from an iterable of raw reason strings.

    Drops blanks and any reason containing a NOBID_JUNK_MARKERS token (e.g.
    'test'); de-duplicates case-insensitively while preserving first-seen order
    and the original casing. Used by the Step 2 guard and the distiller so both
    agree on what counts as a genuine reason.
    """
    seen, out = set(), []
    for raw in reasons:
        r = (raw or "").strip()
        if not r:
            continue
        low = r.lower()
        if any(marker in low for marker in NOBID_JUNK_MARKERS):
            continue
        if low in seen:
            continue
        seen.add(low)
