...
```
Portal Name | Adapter | Direct URL | ID | OCID | Name | Bid Qualification | Bid Qualification Reason(Human) | Created Date
```

Columns are matched to that header **by name**, so the tab may hold any subset of the main sheet's columns in any order. A tab with no header row is skipped with a warning (it syncs 0 rows) rather than failing the run.
...
## Step 7: Maintain the bid knowledge (optional, scheduled separately)

A second, standalone flow collects the team's **manual** bid decisions and distils them into heuristics that feed back into scoring. It is decoupled from the analysis — run it on its own cadence (e.g. weekly), not per analysis run.
...
For this to produce anything, the team must set `Bid Qualification` to `Bid(Human)` / `NoBid(Human)` **and** write a real reason in `Bid Qualification Reason(Human)`. Blank reasons and anything containing `test` are filtered out.

Both artifacts are injected into the analysis prompt as separate precedent blocks — NoBid heuristics push a score **down**, Bid heuristics push it **up**. So whatever this flow writes shapes future scoring directly; that is what the minimums protect against. Deleting a patterns file cleanly disables its block (the loader treats it as absent).
...
└── analyzer/
    ...
    └── knowledge/
        ...
        ├── nobid_patterns.md          # Distilled NoBid heuristics (generated by maintenance)
        └── bid_patterns.md            # Distilled Bid heuristics (generated by maintenance)
```
